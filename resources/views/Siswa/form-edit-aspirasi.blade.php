...
@section('content')
    <div class="main-content min-vh-100">
        <div class="box shadow p-5">
            ...
            <p class="lead fst-italic">Isi data di bawah ini dengan benar!</p>
            <form action="{{ route('siswa.proses-edit') }}" method="POST">
                    ...
                    <div class="col-9">
                        <select class="form-control @error('kategori_id') is-invalid @enderror" name="kategori_id">
                            ...
                            @foreach ($kategori as $item)
                                <option {{ $item->id == $aspirasi->kategori_id ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->nama_kategori }}</option>
                            @endforeach
                        </select>
                        @error('kategori_id')
                ...
                <div class="form-group row mb-3 d-flex align-items-center">
                    ...
                    <div class="col-9">
                        <input  class="form-control @error('judul') is-invalid @enderror" type="text" name="judul" id="judul" value="{{ $aspirasi->judul }}">
                    </div>
                    @error('judul')
                        <div class="invalid-feedback">
                            {{ $message }}
                ...
                <div class="form-group row mb-3 d-flex align-items-center">
                    ...
                    <div class="col-9">
                        <textarea name="isi" id="" rows="10" class="form-control @error('isi') is-invalid @enderror">{{ $aspirasi->id }}</textarea>
                        @error('isi')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
