                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center"><span class="status {{ $item->status }} text-capitalize">{{ $item->status }}</span></td>
                            <td class="text-center">
                                @if ($item->status == 'menunggu')
                                <a href="" class="btn btn-sm btn-success m-0 me-2 tombolEdit" style="width: 120px " data-bs-toggle="modal" data-bs-target="#exampleModal" data-id="{{ $item->id }}">
                                    <i class="fas fa-comment"></i> Tanggapi
                                </a>
                                @else
                                <a href="" class="btn btn-sm btn-info m-0 px-4 me-2 text-white tombolEdit {{ $item->status == 'selesai' || $item->status == 'ditolak' ? 'disabled' : '' }}" style="width: 120px " data-bs-toggle="modal" data-bs-target="#exampleModal" data-id="{{ $item->id }}">
                                    <i class="fas fa-edit"></i> Ubah
                                </a>
                                @endif
                                <a href="/admin/aspirasi/delete/{{ $item->id }}" class="btn btn-sm btn-danger m-0 px-3 me-2 {{ !in_array($item->status, ['selesai', 'ditolak']) ? 'disabled' : '' }}" onclick="return confirm('Yakin aspirasi ini akan dihapus?')">
